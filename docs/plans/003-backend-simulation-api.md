...
Handoff Notes:
- 이 Plan은 API 구현, 테스트, backend Gradle 검증, PR merge 후 master 재검증까지 완료되어 `Done` 상태다.
- 후속 작업은 frontend-backend integration Plan 또는 `004-docker-compose.md` Ready 승격으로 이어간다.
...
## 후속 작업
- frontend Mock UI를 실제 backend API와 연결하는 integration Plan을 작성한다.
- `004-docker-compose.md`는 Plan 003 완료 결과를 기준으로 Ready 여부를 다시 판단한다.
